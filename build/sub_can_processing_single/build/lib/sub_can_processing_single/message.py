...
class Message():
    ...
    def _to_ros(self) -> Dict:
        return {CAN_KEYS_TO_ROS_KEYS[k]: getattr(self, k).to_ros() for k in self.get_avail_keys()}
    ...
    def from_ros(cls, msg) -> "Message":
        ...
        if len(getattr(msg, "objects")) > 0:
            kwargs.update(dict(
                Object_TrackA=[], Object_TrackB=[], Object_TrackC=[], Object_TrackD=[]
            ))
