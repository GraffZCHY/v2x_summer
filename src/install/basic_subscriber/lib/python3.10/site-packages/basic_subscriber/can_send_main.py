...
class SendCANMessage:
    ...
    def encode_send(self, db, can_bus, fd_out):
        ...
        for key in msg:
            ...
            elif key == "V2XIntersection_MAP":
                print("V2X msg detected.")
                data = db.encode_message(
                    "V2XIntersection_MAP",
                    {
                        "IntersectionID": int(
                            msg["V2XIntersection_MAP"][i]["IntersectionID"]
                        ),
                        "IngressLaneID": int(
                            msg["V2XIntersection_MAP"][i]["IngressLaneID"]
                        ),
                        "PlannedEgressLaneID": int(
                            msg["V2XIntersection_MAP"][i]["PlannedEgressLaneID"]
                        ),
                        "PlannedManeuver": int(
                            msg["V2XIntersection_MAP"][i]["PlannedManeuver"]
                        ),
                        "SignalGroup": int(
                            msg["V2XIntersection_MAP"][i]["SignalGroup"]
                        ),
                        "Rolling_Count": self.Rolling_Count,
                        "MAP_Validity": int(
                            msg["V2XIntersection_MAP"][i]["MAP_Validity"]
                        ),
                    },
                )
                message = can.Message(
            ...
            elif key == "V2XIntersection_SPAT":
                data = db.encode_message(
                    "V2XIntersection_SPAT",
                    {
                        "IntersectionID": int(
                            msg["V2XIntersection_SPAT"][i]["IntersectionID"]
                        ),
                        "SignalGroup": int(
                            msg["V2XIntersection_SPAT"][i]["SignalGroup"]
                        ),
                        "SPAT_Timestamp": int(
                            msg["V2XIntersection_SPAT"][i]["SPAT_Timestamp"]
                        ),
                        "SecondsUntilChangeMinimum": int(
                            msg["V2XIntersection_SPAT"][i][
                                "SecondsUntilChangeMinimum"
                            ]
                        ),
                        "SecondsUntilChangeMaximum": int(
                            msg["V2XIntersection_SPAT"][i][
                                "SecondsUntilChangeMaximum"
                            ]
                        ),
                        "MovementEventState": int(
                            msg["V2XIntersection_SPAT"][i]["MovementEventState"]
                        ),
                        "Rolling_Count": self.Rolling_Count,
                        "SPAT_Validity": int(
                            msg["V2XIntersection_SPAT"][i]["SPAT_Validity"]
                        ),
                    },
                )
                message = can.Message(
